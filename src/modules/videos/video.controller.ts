...
export async function updateVideoHandler(
  ...
  const { _id: userId } = res.locals.users;
...
export async function findVideosHandler(_: Request, res: Response) {
  ...
  if (!videos) {
    return res.status(StatusCodes.NOT_FOUND).send("Videos not found");
  }
...
export async function streamVideoHandler(req: Request, res: Response) {
  ...
  if (!range) {
    return res.status(StatusCodes.BAD_REQUEST).send("Range should be provide");
  }
